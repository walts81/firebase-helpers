...
function initFirebaseArray(dbRef, arr, commit, mutation, firebaseConfig) {
    return __awaiter(this, void 0, void 0, function* () {
        const configToUse = !!commit && typeof commit === 'object' ? commit : firebaseConfig;
        yield service.remove(dbRef, configToUse);
        for (const item of arr) {
            const snapshot = yield service.push(dbRef, item);
            item.key = snapshot.key;
            const path = service.getPath(dbRef, item.key);
            const newRef = service.ref(path, configToUse);
            yield service.update(newRef, item, configToUse);
        }
